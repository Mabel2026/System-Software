...
void main() { 
    int prog_len, length, objc_len, text_rec_len = 0, 
text_rec_addr; 
    char start_addr[10], loc[10], label[10], opcode[10], 
operand[10]; 
    ...
    if (!intermediate_ptr || !optab_ptr || !symtab_ptr || 
!size_ptr || !object_ptr) { 
        printf("Error: Could not open one or more files.\n"); 
        exit(1); 
    } 
 
    fscanf(intermediate_ptr, "%s %d %s %s %s", loc, &length, 
label, opcode, operand); 
    ...
    printf("H^%s^%06X^%06X\n", label, (int)strtol(start_addr, 
NULL, 16), prog_len); 
    fprintf(object_ptr, "H^%s^%06X^%06X\n", label, 
(int)strtol(start_addr, NULL, 16), prog_len); 
    snprintf(text_header, sizeof(text_header), "T^%06X", 
text_rec_addr); 
...
label, opcode, operand); 
        ...
        while (fscanf(optab_ptr, "%s %s", opc, val) != EOF) { 
            char newopc[20] = "+"; 
            strcat(newopc, opc); 
            if (strcmp(opcode, opc) == 0 || strcmp(opcode, 
newopc) == 0) { 
                int operand_found = 0; 
                fseek(symtab_ptr, 0, SEEK_SET); 
                while (fscanf(symtab_ptr, "%s %s", sym_name, 
sym_addr) != EOF) { 
                    if (strcmp(operand, sym_name) == 0) { 
                        snprintf(temp_objc, sizeof(temp_objc), 
"^%s%s", val, sym_addr); 
                        objc_len = 3; 
                        operand_found = 1; 
                        break; 
       ...
       if (strcmp(opcode, "BYTE") == 0) { 
            ...
            for (int i = 2; i < strlen(operand) - 1; i++) { 
                char hex_val[3]; 
                snprintf(hex_val, sizeof(hex_val), "%02X", 
operand[i]); 
        ...
        if (strcmp(opcode, "WORD") == 0) { 
            snprintf(temp_objc, sizeof(temp_objc), "^%06X", 
(int)strtol(operand, NULL, 10)); 
            objc_len = 3; 
        } 
