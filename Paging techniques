...
scanf("%d", &size); 
int required_frames = (size + frame_size - 1) / 
frame_size; 
...
p->p_size = size;p->frame_count = 
required_frames; 
p->internal_frag =(frame_size * 
required_frames) - size; 
...
int index = -1; 
for (int i = 0; i < 
process_count; i++) {if (strcmp(processes[i].p_name, p1) == 0) { 
index = i; 
break; 
} 
} 
